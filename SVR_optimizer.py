...
class SVR_optimizer():
    def __init__(self, src_folder:str, prep_folder:str, result_folder:str, stack_filenames:list, mask_filename:str, pixdims:list, device:str, PSF, loss_kernel_size, monai_mode:str, tio_mode:str, roi_only:bool=False, lr_vol_vol:float = 0.0035, tensorboard_path = "")->None:
        timer = time.time()
        ...
        Args:
            ...
            loss_kernel_size(int/str): kernel size for loss function (max/int)
        ...
        self.loss_kernel_size = loss_kernel_size
        ...
        self.fixed_image, self.stacks, self.slice_dimensions, self.rot_params_init, self.trans_params_init = self.svr_preprocessor.preprocess_stacks_and_common_vol(self.pixdims[0], PSF, roi_only=roi_only, lr_vol_vol=lr_vol_vol, tensorboard_path = tensorboard_path)
    ...
    def optimize_volume_to_slice(self, epochs:int, inner_epochs:int, lr, PSF, lambda_scheduler, loss_fnc = "ncc", opt_alg = "Adam", tensorboard:bool = False, tensorboard_path = '', from_checkpoint:bool=False, last_rec_file:str='', last_epoch:int=0):
        ...
        for epoch in range(first_epoch,first_epoch + epochs):
            ...
            if epoch < epochs - 1:
                ...
                """
                _ = self.svr_preprocessor.save_intermediate_reconstruction_and_upsample(common_volume_pure, fixed_image_meta, int('9' + str(epoch)), upsample=upsample_bool, pix_dim = self.pixdims[epoch+1])
                """
    ...
    def prepare_optimization(self, PSF, loss_kernel_size, lambda1, opt_alg, loss_fnc, lr):
        """
        Prepar optimization, generate slices, load models, optimizers and scheduler

        Args:
            PSF (_type_): Point spread function
            loss_kernel_size(int/str): kernel size of local ncc loss
            lambda1 (lambda-expression): rules the learning rate schedule
